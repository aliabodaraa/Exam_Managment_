...
@section('content')
    <div class="bg-light p-2 rounded">
        <h1 class="text-center"> تعديل إعتراضات <mark>{{ $user->username }}</mark> في {{ $rotation->name }}  - {{ $rotation->year }} </h1>
        <div class="container-fluid p-2 rounded">
            <div class="mt-2">
                @include('layouts.partials.messages')
            </div>
            @if(count($courses_info))
            <form method="POST" action="{{route('rotations.objections.update',[$rotation,$user])}}">
                @method('patch')
                @csrf
                                            ...
                                            @if($courseQ)
                                                <td id="course{{ $courseQ->id }}" class="course" align="center" height="100">
                                                    ...
                                                    height: 40px;">
                                                        ...
                                                        value="{{ $courseQ->id }}"
                                                        class="course_in_day{{$counter}} toggler-wrapper style-3 course_{{ $dateTime }}" onclick="selectCourseFunction(JSON.parse(JSON.stringify({{ json_encode($dateTime) }})),event)" {{(in_array($courseQ->id,$courses_objections_ids)?'checked':'')}}>

                                                        <h5 class='course-name' onclick="selectCourseFunction({{ $courseQ->id }})" style="white-space: pre-wrap;">
                                                            @php
                                                                if($courseQ)
    ...
    $(document).ready(function(){
        ...
        for (let counter = 0; counter < 80; counter++) {
            $.each($('.course_in_day'+counter), function() {
                if($('.course_in_day'+counter).is(':checked')){
                    $('#day'+counter).prop('checked',true);
                    console.log("dd");
                }
            });
        }
        ...
        select_all_courses_in_day=(counter)=>{
                if($('#day'+counter).is(':checked')) {
                    ...
                    $.each($('.course_in_day'+counter), function() {
                       ...
                       //$('#day'+counter).prop('checked',false);
                    });
                }
